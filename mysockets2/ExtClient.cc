...
void ExtClient::initialize()
{
    ...
    scheduleAt(simTime()+1.0, refresher);
}
...
void ExtClient::handleSocketEvent2()
{
    // try to find a double line feed in the input -- that's the end of the HTTP header.
    char *endHeader = nullptr;
    for (char *s = recvBuffer2; s <= recvBuffer2+numRecvBytes2-4; s++)
        if (*s == '\r' && *(s+1) == '\n' && *(s+2) == '\r' && *(s+3) == '\n') {
            endHeader = s+4;
            break;
        }
    ...
    if (!endHeader)
        return;
    std::string header = std::string(recvBuffer2, endHeader-recvBuffer2);
    // EV << header;
...
void ExtClient::processHTTPReply(HTTPMsg *httpMsg)
{
    delete httpMsg;
}
